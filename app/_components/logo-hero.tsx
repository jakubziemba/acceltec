...
export default function LogoHero({ className = "" }) {
  let mouseX = useMotionValue(485);
  let mouseY = useMotionValue(84);
  let mouseXSpring = useSpring(mouseX, {
    // found this settings more natural than the default
    damping: 25,
    mass: 0.5,
  });
  let mouseYSpring = useSpring(mouseY, {
    damping: 25,
    mass: 0.5,
  });
  ...
  const gradientTransform = useMotionTemplate`translate(${mouseXSpring} ${mouseYSpring}) rotate(112.237) scale(350.073 450.056)`;
    ...
    >
      ...
      <defs>
        ...
        >
          ...
          <stop offset={1} stopColor="black" stopOpacity={0.02} />
          <stop offset={2} stopColor="white" stopOpacity={0.01} />
        </motion.linearGradient> */}
      </defs>
    </motion.svg>
  );

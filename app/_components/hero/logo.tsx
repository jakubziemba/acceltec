...
export default function LogoHero({ className = "" }) {
  ...
  let gradientMove = useMotionTemplate`translate(${mouseXSpring} ${mouseYSpring}) rotate(112.237) scale(350.073 550.056)`;

  function handleMouseMove({ currentTarget, clientX, clientY }: MouseEvent) {
    if (!currentTarget || !ref.current) return;

    let { left, top, right, bottom } = ref.current.getBoundingClientRect();
    const svgWidth = right - left;
    const svgHeight = top - bottom;
    const xPosition = Math.min(svgWidth, Math.max(0, clientX - left)); // Limit xPosition to svgWidth
  ...
  return (
    <div
      ...
      className="relative top-0 flex h-full w-full items-center justify-center pb-24 pt-20 lg:px-24 lg:pt-24"
    >
      <motion.svg
        ref={ref}
        ...
        className={tw("h-full w-full min-w-[890px] max-w-[1334px]", className)}
      >
        <motion.path
